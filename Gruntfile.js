	...
	grunt.initConfig({
		dojo: {
			dist: {
				options: {
					...
					profile: path.join('profiles', 'dicomappweb.profile.js'),
		...
		stylus: {
			compile: {
				...
				files: {
					'src/dicomappweb/resources/main.css': 'src/dicomappweb/resources/main.styl'
				}
			}
		},
		watch: {
			stylus: {
				files: 'src/dicomappweb/resources/**/*.styl',
		...
		intern: {
			local: {
				options: {
					runType: 'client',
					config: 'src/dicomappweb/tests/intern'
				}
			},
			remote: {
				options: {
