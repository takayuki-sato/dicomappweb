...
define({
	...
	loader: {
		...
		packages: [
			...
			{ name: 'dicomappweb', location: 'src/dicomappweb' }
		]
	},

	// Non-functional test suite(s) to run in each browser
	suites: [ 'dicomappweb/tests/unit' ],

	// Functional test suite(s) to run in each browser once non-functional tests are completed
	functionalSuites: [ 'dicomappweb/tests/functional' ],

	// A regular expression matching URLs to files that should not be included in code coverage analysis
	excludeInstrumentation: /^(?:node_modules|src\/(?:dojo|dijit|dojox|put-selector|xstyle|dgrid|dicomappweb\/tests))\//
});
